...
- STM32F103RB 레지스터 직접 설정 방식으로 GPIO, Timer, ADC, DMA, USART, NVIC 초기화 코드를 구성했습니다.
- TIM1, TIM2, TIM3, TIM4의 역할을 분리하여 display refresh, 온도값 갱신, 시간/timeout 관리, keypad scan을 각각 담당하도록 설계했습니다.
- Keypad 입력을 기능별 Queue로 분리하여 일반 입력, 시간 설정, 인증값 설정, 인증값 비교 상태가 충돌하지 않도록 구현했습니다.
- PB3/PB4 JTAG 기능 충돌, Dot Matrix 일부 핀 불량, Keypad bouncing 문제를 직접 추적하고 우회했습니다.
- 하드웨어 제약이 있는 상황에서도 타이머, 온도 표시, 인증/잠금 기능이 유지되도록 출력 구조를 조정했습니다.
...
| 구간 | 읽을 포인트 |
|---|---|
| 초기화 함수 | GPIO, Timer, ADC/DMA, USART, NVIC 설정 흐름 확인 |
| Queue 관련 코드 | 여러 입력 모드를 어떻게 분리했는지 확인 |
| TIM1 interrupt | Dot Matrix multiplexing 출력 방식 확인 |
| TIM2 interrupt | ADC/DMA 기반 온도값 갱신 흐름 확인 |
| TIM3 interrupt | 시계 카운트, timeout, 상태 표시 시간 관리 확인 |
| TIM4 interrupt | Keypad row/column scan 및 debounce 처리 확인 |

이 코드는 특정 실습 보드와 Dot Matrix 배선에 의존합니다. 따라서 저장소에서는 전체 재현보다 **주변장치 설정 방식, interrupt 분리 구조, 하드웨어 문제 대응 방식**을 확인하는 용도로 보는 것이 적절합니다.

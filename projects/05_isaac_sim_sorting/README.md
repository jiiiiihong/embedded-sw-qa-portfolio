...
- Isaac Sim `finalfac1.usd` 환경에서 QR 부착 박스가 컨베이어로 공급되는 물류 시나리오를 구성했습니다.
- `packages_*.csv`의 `qr_id`, `route_zone`을 읽어 출고일 기준 target을 결정하는 라우팅 로직을 구현했습니다.
- robot1 상단 카메라로 QR을 읽고, decode 실패 시 prim attribute를 fallback으로 사용하는 구조를 만들었습니다.
- box asset의 QR 면 방향 문제를 추적해 local `-Y` 면이 위로 오도록 회전값을 강제했습니다.
- robot1은 today/not-today 1차 분류, robot2는 day2/day3 2차 분류를 담당하도록 역할을 분리했습니다.
- final gate 판정을 center inside에서 bbox overlap 방식으로 바꿔 실제 컨베이어 흐름에 가까운 성공 조건을 만들었습니다.
- 큰 box asset이 로봇 동작을 방해하지 않도록 collision proxy 개념을 적용했습니다.
...
| 구간 | 읽을 포인트 |
|---|---|
| CSV loading / route mapping | QR payload와 route zone을 어떻게 target으로 바꾸는지 확인 |
| box spawning | QR이 포함된 box asset을 어떻게 선택하고 배치하는지 확인 |
| QR camera / decode | robot1 camera capture, OpenCV QR decode, debug image 저장 흐름 확인 |
| fallback logic | 실제 decode 실패 시 `user:qr_payload`를 어떻게 사용하는지 확인 |
| robot1 sorting | today / not-today 1차 분류 조건 확인 |
| robot2 sorting | day2 / day3 2차 분류 조건 확인 |
| final gate check | center inside 대신 bbox overlap을 적용한 이유 확인 |
| physics handling | asset collision과 proxy collider를 어떻게 다루는지 확인 |

이 코드는 Isaac Sim, USD asset, box asset, 카메라 설정에 강하게 의존합니다. 저장소에서는 전체 재현보다 **센서 인식, fallback, 다중 로봇 분기, 최종 판정 조건을 통합한 구조**를 중심으로 보는 것이 좋습니다.
